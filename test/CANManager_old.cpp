...
bool CANManager::initializeBus(uint8_t busNum, uint32_t speed) {
    switch (busNum) {
        case 1:
            ...
            // Keep it simple - no FIFO or interrupts
            delay(100);
            
            Serial.printf("\r\n  CAN1 actual baud: %lu", can1.getBaudRate());
            Serial.print("\r\n  CAN1 using simple polling mode (no interrupts)");
        ...
        case 2:
            ...
            Serial.printf("\r\n  CAN2 actual baud: %lu", can2.getBaudRate());
            Serial.print("\r\n  CAN2 using simple polling mode (no interrupts)");
        ...
        case 3: {
            ...
            // V6-NG uses simple polling, no interrupts or FIFO for CAN3
            delay(100);
            
            Serial.printf("\r\n  CAN3 actual baud: %lu", can3.getBaudRate());
            Serial.print("\r\n  CAN3 using simple polling mode (no interrupts)");
...
void CANManager::can3MessageHandler(const CAN_message_t &msg) {
    // Not used - CAN3 uses polling mode
    if (instance) {
...
void CANManager::processCANMessage(uint8_t busNum, const CAN_message_t &msg) {
    ...
    switch (busNum) {
        ...
        case 3:
            ...
            // Check for Keya motor messages
            if (msg.id == 0x07000001) {  // Heartbeat
            ...
            } else if (msg.id == 0x05800001) {  // Acknowledge
                // Keya acknowledge message - could track these if needed
                static uint32_t ackCount = 0;
                if (++ackCount < 5) {
                    Serial.printf("\r\n[CAN3] Keya ACK received");
                }
            }
            break;
    }
...
bool CANManager::sendMessage(uint8_t busNum, const CAN_message_t &msg) {
    ...
    switch (busNum) {
        ...
        case 3:
            Serial.printf("\r\n[CAN] Sending on CAN3: initialized=%d, busOff=%d", 
                         can3Info.initialized, can3Info.busOff);
            if (can3Info.initialized && !can3Info.busOff) {
                Serial.print("\r\n[CAN] Calling can3.write()...");
                int writeResult = can3.write(msg);
                Serial.printf("\r\n[CAN] can3.write() returned: %d", writeResult);
                success = (writeResult == 1);
