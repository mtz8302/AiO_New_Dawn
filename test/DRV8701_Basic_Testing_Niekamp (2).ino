...
#define SLEEP_PIN 4  // LOCK output, DRV sleep pin on rev D and earlier, not used on rev E+
...
void setup() {
  ...
  pinMode(SLEEP_PIN, OUTPUT); // not used/needed on rev E+
  ...
  digitalWrite(SLEEP_PIN, HIGH);  // LOW keeps DRV8701-Cytron asleep on rev D and earlier?
...
void loop() {
  ...
  if (asLoop > 100) {
    ...
    int16_t pwmOut = constrain(map(readWAS, 51, 978, -256, 256), -256, 256);  // map WAS input 51-978 -> 0-256 PWM output
    Serial.printf(" PWM: %3i(%03i%)", pwmOut, int(float(pwmOut) / 2.55));

    const bool PWM_POLARITY = 0;  // Sets the motor Off/Center/Zero output PWM polarity
                                  // 0 - 0-256 duty cycle
                                  // 1 - 256-0 duty cycle

    if (pwmOut < 0 && outputActive) {
      analogWrite(PWM1_PIN, (PWM_POLARITY) ? 256 + pwmOut : -pwmOut);
      analogWrite(PWM2_PIN, (PWM_POLARITY) ? 256 : 0);
    } else if (pwmOut > 0 && outputActive) {
      analogWrite(PWM1_PIN, (PWM_POLARITY) ? 256 : 0);
      analogWrite(PWM2_PIN, (PWM_POLARITY) ? 256 - pwmOut : pwmOut);
    } else {
      analogWrite(PWM1_PIN, (PWM_POLARITY) ? 256 : 0);
      analogWrite(PWM2_PIN, (PWM_POLARITY) ? 256 : 0);
    }
    ...
    currentAve /= 100;
    uint16_t readCurrent = abs(analogRead(CURRENT_PIN) - 22); // -22 to adjust for Zero current offset on rev E+
  ...
  if (csLoop > 1) {
    csLoop = 0;
    uint16_t readCurrent = abs(analogRead(CURRENT_PIN) - 22);
